...
public class OnmsgchkServiceImpl implements OnmsgchkService {
    ...
    private HttpHeaders responseHeaders; // 응답 header
    ...
    public ResponseEntity<OnmsgchkOutputVO> syncCall(HttpHeaders requestHeaders, OnmsgchkInputVO inputVO) {
        ...
        commonInputDataValidChk(requestHeaders, inputVO);
        ...
        if (!StringUtils.isEmpty(rul_svcavl_con_tpcom_vs2001OutputVO.getCncl_yn()) &&
            ...
            if (inputVO.getMcht_biz_no().length() != 10) {
                throw new ValidException(requestHeaders, "3311", "사업자번호 미입력 또는 길이 오류 [" + inputVO.getMcht_biz_no() + "]");
            }
        } else {    // 취소 전문인 경우
            if (StringUtils.isEmpty(inputVO.getOrgn_deal_dy())) {
                throw new ValidException(requestHeaders, "7745", "원거래일자 미입력");
            }
            if ((StringUtils.isEmpty(inputVO.getOrgn_deal_aprv_no()) && StringUtils.isEmpty(inputVO.getOrgn_deal_coopco_aprv_no())) &&
                ...
                throw new ValidException(requestHeaders, "7746", "원거래승인번호/원거래제휴사승인번호 미입력");
            }
            if (StringUtils.isEmpty(inputVO.getCncl_req_fg())) {
                throw new ValidException(requestHeaders, "3313", "요청 서비스구분 미입력");
            }
        }
        responseHeaders = new HttpHeaders();
        return new ResponseEntity<OnmsgchkOutputVO>(outputVO, responseHeaders, HttpStatus.OK);
    }


    /* 공통 입력값 유효성 체크*/
    public void commonInputDataValidChk(HttpHeaders requestHeaders, OnmsgchkInputVO inputVO) {
        if (StringUtils.isEmpty(inputVO.getTrc_no())) {
            throw new ValidException(requestHeaders, "7777", "추적번호 미 유입");
        }
        if (StringUtils.isEmpty(inputVO.getMcht_no())) {
            throw new ValidException(requestHeaders, "7730", "가맹점번호 미 유입");
        }
        if (StringUtils.isEmpty(inputVO.getDeal_dy())) {
            throw new ValidException(requestHeaders, "6940", "거래일자 미 유입");
        }
        if (StringUtils.isEmpty(inputVO.getCrd_no()) &&
                StringUtils.isEmpty(inputVO.getResd_no())) {
            throw new ValidException(requestHeaders, "7720", "식별자 미 유입");
        }
    }
}
